...
interface HeaderProps {
  ...
  isLoggingIn?: boolean;
  onReturnToRift?: () => void;
  onAdminTeleport?: (spotId: number) => void;
}
...
export default function Header({ 
  ...
  const [adminSelectedSpot, setAdminSelectedSpot] = useState<number>(SPOTS[0]?.id || 2);
  ...
  return (
    <>
      <header className="relative z-30 p-3 sm:p-4 bg-black/40 backdrop-blur-xl border-b border-purple-500/20 shadow-2xl transition-all duration-300">
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button 
              ...
              className={`text-[10px] font-bold px-2.5 py-1.5 rounded-xl border transition backdrop-blur-md ${
                isAdmin 
                  ? 'bg-red-950/80 border-red-500 text-red-300 shadow-red-500/30 shadow animate-pulse' 
        ...
        {/* Admin Hyper-Teleport Bar (Recycled Dropdown for instant movement) */}
        {isAdmin && (
      ...
      {isAdminAuthModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-gray-950 border border-red-500/50 rounded-3xl p-6 max-w-sm w-full shadow-2xl shadow-red-950/50 flex flex-col space-y-4 text-left">
            ...
            <form onSubmit={handleAdminAuthSubmit} className="flex flex-col space-y-3">
              <input 
                ...
                autoFocus
                className="w-full bg-black/60 border border-red-500/40 rounded-xl px-3.5 py-2.5 text-white font-mono text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
              />
