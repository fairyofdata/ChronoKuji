...
export default function PwaInstallBanner({ onInstalled }: PwaInstallBannerProps) {
  ...
  const bannerTitle = language === 'en'
    ? 'Install App & Get Bonus Tokens'
    : language === 'ja'
    ? 'アプリをインストールしてボーナストークン獲得'
    : '앱으로 설치하고 보너스 토큰 받기';

  const bannerDesc = language === 'en'
    ? 'Install to home screen for 1 free bonus token!'
    : language === 'ja'
    ? 'ホーム画面に追加するとボーナストークンが1回付与されます！'
    : '홈 화면에 설치하면 1회 보너스 토큰이 지급됩니다!';

  const installBtn = language === 'en'
    ? 'Install'
    : language === 'ja'
    ? 'インストール'
    : '설치';
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-40 max-w-md mx-auto bg-gradient-to-r from-purple-950/95 via-gray-900/95 to-indigo-950/95 border border-purple-500/50 backdrop-blur-xl p-4 rounded-2xl shadow-2xl flex items-center justify-between text-white animate-bounce-slow">
      <div className="flex items-center space-x-3 text-left">
        ...
        <div>
          <h4 className="text-xs font-black text-amber-300">{bannerTitle}</h4>
          <p className="text-[10px] text-gray-300">{bannerDesc}</p>
        </div>
      </div>
